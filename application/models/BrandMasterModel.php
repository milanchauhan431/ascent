...
class BrandMasterModel extends MasterModel{
    ...
    public function save($data){
        try{
            ...
            if($this->checkDuplicate($data) > 0):
                return ['status'=>0,'message'=>['brand_name'=>"Make Name is duplicate."]];
            endif;
    ...
    public function delete($id){
        try{
            ...
            $brandData = $this->getBrand(['id'=>$id]);

            $checkData['columnName'] = ["make_brand","make"];
            $checkData['value'] = $brandData->brand_name;
            $checkUsed = $this->checkUsage($checkData);

            if($checkUsed == true):
                return ['status'=>0,'message'=>'The make is currently in use. you cannot delete it.'];
            endif;
